...
WEIGHT_FINGER_REPEATS = 1024 #: Cost of a finger repeat. Gets additional +1 from the row change on the same finger.
...
WEIGHT_BIGRAM_ROW_CHANGE_PER_ROW = 40 #: When I have to switch the row in a bigram while on the same hand, that takes time => Penalty per (row to cross ² / horizontal distance)² if we’re on the same hand. 
...
WEIGHT_SECONDARY_BIGRAM_IN_TRIGRAM = 0.1 #: multiplier for the cost of secondary bigrams in trigrams. 
...
COST_PER_KEY  = [
    ...
    [24,    10, 6, 4, 6, 9,    10, 6, 4, 5, 8,8,36, 0], # Reihe 1
    [5,      3, 3, 3, 3, 5,     5, 3, 3, 3, 3, 5,10,18], # Reihe 2
    [15,12, 16,24,20, 6,    30, 7, 5,22,22,13,      15],     # Reihe 3
