...
echo … final penalty 
for j in {1..9} {1..9}0 {1..9}00 {1..9}000 {1..3}0000; do grep "billion total" results/$1-evolve-$j-results.txt | cut -d " " -f 2| sed "s/^/$j /" ; done > results/$1-evolve-range-final-penalty.txt 
echo … finger repeats
...
for j in {1..9} {1..9}0 {1..9}00 {1..9}000 {1..3}0000; do grep handswitching results/$1-evolve-$j-results.txt | cut -d " " -f 2| sed "s/^/$j /" ; done > results/$1-evolve-range-handswitching.txt
