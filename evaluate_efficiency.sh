...
echo create data
for j in {1..130} {200..205} {300..305} {400..405} {600..603} {1000..1003} 10000 ; do time ./check_neo.py --evolve $j --prerandomize 10000 -q >> results/2010-04-27-evolve-$j-results.txt; done

echo check and plot the data
echo … final penalty 
for j in {1..130} {200..205} {300..305} {400..405} {600..603} {1000..1003} 10000; do grep billion results/2010-04-27-evolve-$j-results.txt | cut -d " " -f 2| sed "s/^/$j /" ; done > results/2010-04-27-evolve-range-final-penalty.txt 
echo … finger repeats
