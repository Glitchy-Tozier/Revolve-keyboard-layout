...
For 1 page it would be around 0.05 / 360, or 1.4×10\(^{-4}\). Note that the tail here is cut, because Google ngrams starts at 40 matches per year.
...
#+BEGIN_SRC python
  ...
  required = int(nmatches * (1 - 1.4e-4))
  words = []
  matches = 0
  ...
  import pylab
  pylab.plot(pylab.cumsum([j for i,j in matchcounts.most_common()]))
  pylab.show()
